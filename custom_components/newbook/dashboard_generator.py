...
class DashboardGenerator:
    ...
    async def async_generate_all_dashboards(self, rooms: dict[str, dict[str, Any]]) -> None:
        """Generate all dashboards based on discovered rooms."""
        _LOGGER.info("Generating Newbook dashboards for %d rooms", len(rooms))

        # Ensure dashboards directory exists
        await self.hass.async_add_executor_job(self._ensure_directory)

        # Generate each dashboard type
        try:
            await self._async_generate_home_overview(rooms)
            await self._async_generate_room_dashboards(rooms)
            await self._async_generate_battery_dashboard(rooms)
            await self._async_generate_health_dashboard(rooms)

            _LOGGER.info("Successfully generated all Newbook dashboards")
        except Exception as err:
            _LOGGER.error("Error generating dashboards: %s", err, exc_info=True)
    ...
    async def _async_generate_home_overview(self, rooms: dict[str, dict[str, Any]]) -> None:
        ...
        for room_id, room_info in sorted_rooms:
            ...
            card = {
                ...
                "tap_action": {
                    "action": "navigate",
                    "navigation_path": f"/dashboards-newbook/room-{normalized_id}",
                },
