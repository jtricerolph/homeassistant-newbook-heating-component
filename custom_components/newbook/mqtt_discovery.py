...
class MQTTDiscoveryManager:
    ...
    async def _async_publish_diagnostic_sensors(
        ...
        update_config = {
            ...
            "stat_t": f"shellies/{device.device_id}/info",
            "value_template": "{{ 'ON' if (value_json.update.has_update | default(false)) else 'OFF' }}",
            "payload_on": "ON",
            "payload_off": "OFF",
            "device_class": "update",
            "entity_category": "diagnostic",
            "json_attributes_topic": f"shellies/{device.device_id}/info",
            "json_attributes_template": '{{ {"status": value_json.update.status, "new_version": value_json.update.new_version, "old_version": value_json.update.old_version} | tojson }}',
            "device": {
                "identifiers": [f"shelly_{device.mac}"],
            },
        }
